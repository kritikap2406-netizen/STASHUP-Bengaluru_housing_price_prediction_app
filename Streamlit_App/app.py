...
<style>
    ...
    .price-banner {
        background: linear-gradient(135deg, #312e81 0%, #6d28d9 50%, #4c1d95 100%);
        padding: 28px;
        border-radius: 18px;
        color: white;
        text-align: center;
...
def render_metric(label, value, sub=None, accent=None):
    sub_html = f'<div style="font-size:.8rem;color:#8b85a3;margin-top:4px">{sub or ""}</div>'
    color = accent or "#fff"
    st.markdown(f"""
...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        default_index=0,
        styles={
            "container": {"padding": "4px", "background-color": "#14121f", "border-radius": "10px"},
            "icon": {"color": "#c4b5fd", "font-size": "16px"},
            "nav-link": {"color": "#d8d4e8", "font-size": "14px", "padding": "10px 12px", "border-radius": "8px"},
            "nav-link-selected": {"background-color": "#6d28d9", "color": "#fff", "font-weight": "600"},
        },
    )
    ...
    st.markdown(f"""
    <div style='font-size:.82rem;color:#8b85a3;line-height:1.5'>
...
if selected == "Predict":
    ...
    with c2:
        ...
        if total_sqft > 0:
            ...
            <div class='metric-card'>
              ...
              <div style='font-size:.8rem;color:#8b85a3;margin-top:4px'>
    ...
    if predict_btn:
        ...
        st.markdown(f"""
        <div style='font-size:.85rem;color:#8b85a3;text-align:center;margin-top:.5rem'>
...
# ------------------------------------------------------------------
    ...
    if st.button("🔬 Compare Predictions", type="primary", use_container_width=True):
        ...
        st.markdown(f"""
        <div class='price-banner' style='background:linear-gradient(135deg,#4c1d95,#4338ca)'>
